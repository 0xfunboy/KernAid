//! Linux mutation primitives restricted to an explicitly marked disposable
//! fixture. This module is compiled only by `fixture-repair-lab`.
...
/// Read-only rollback preflight. It exposes only opaque fingerprints and
/// never the trusted local backup path carried by [`RepairReceipt`].
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct FstabRollbackPreview {
    pub target_fingerprint: String,
...
/// Verify the exact installed state and its byte-verified backup without
/// mutating either one. Callers cannot supply a path independently of the
/// trusted, completed repair receipt.
pub fn preview_missing_fstab_device_rollback(
    root: &Path,
    repair: &RepairReceipt,
) -> Result<FstabRollbackPreview, RepairError> {
    let fixture = canonical_fixture(root)?;
    let installed = snapshot_named(&fixture.etc, Path::new(FSTAB_NAME))?;
    if fingerprint(&installed.bytes) != repair.after_fingerprint
        || snapshot_precondition(&installed) != repair.after_target_precondition
        || installed.state.metadata != repair.before_metadata
    {
        return Err(RepairError::StaleTarget);
    }
    let (_backup_parent, _backup_fd, backup) = open_verified_backup(&fixture, repair)?;
    Ok(FstabRollbackPreview {
        target_fingerprint: snapshot_precondition(&installed),
        installed_sha256: fingerprint(&installed.bytes),
        restored_sha256: fingerprint(&backup.bytes),
        backup_sha256: fingerprint(&backup.bytes),
        validation: "restored fstab bytes and original mode/uid/gid match the verified backup",
    })
}
...
mod tests {
    ...
    fn repair_is_backed_up_validated_and_rollbackable() {
        ...
        let rollback_preview = preview_missing_fstab_device_rollback(&tree.target(), &receipt)
            .expect("preview explicit rollback");
        assert_eq!(
            rollback_preview.target_fingerprint,
            receipt.after_target_precondition
        );
        assert_eq!(rollback_preview.installed_sha256, receipt.after_fingerprint);
        assert_eq!(rollback_preview.restored_sha256, receipt.before_fingerprint);
        assert_eq!(rollback_preview.backup_sha256, receipt.backup_fingerprint);
