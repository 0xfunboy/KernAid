...
// This module preserves the already-shipped Desk normalizer API while moving
// it into the shared pack; changing every `Result<_, ()>` caller is outside
// this R0 extraction and would create avoidable protocol drift.
...
mod tests {
    ...
    const STORAGE: &str = include_str!("../fixtures/native/system-profiler-storage.json");
    const APFS: &[u8] = include_bytes!("../fixtures/native/apfs-list.plist");
    const ROOT: &[u8] = include_bytes!("../fixtures/native/root-info.plist");
    const LAUNCHD_USER: &str = include_str!("../fixtures/native/launchd-user.txt");
    const NWI: &str = include_str!("../fixtures/native/network-nwi.txt");
    const DNS: &str = include_str!("../fixtures/native/network-dns.txt");
    const SNAPSHOTS: &str = include_str!("../fixtures/native/snapshots.txt");
