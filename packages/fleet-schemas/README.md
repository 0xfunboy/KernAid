...
kernaid:fleet:work-order-claim:v1\0 || canonical JSON(claim without signature)
kernaid:fleet:work-order-result:v1\0 || canonical JSON(result without signature)
...
The fixed update vector is generated by `kernaid-update-client` from a public
test seed and verifies the manifest canonical bytes, direct domain framing and
Ed25519 signature byte-for-byte in TypeScript.
...
- `dev.kernaid.update.manifest.v1` is the closed vendor-signed artifact
  descriptor accepted by `kernaid-update-client`; it contains no signing key,
...
- `dev.kernaid.fleet.service-receipt.v1` binds one accepted device operation to
  ...
  It also acknowledges work-order claim/result transport, but never grants
  local execution authority.
- Work-order claims and results are signed by the enrolled device. The catalog
  contains only bounded collector/action IDs with an exact version; requests
