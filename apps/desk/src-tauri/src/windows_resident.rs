...
use kernaid_windows_pack::diagnostics::EvidenceInput;
#[cfg(test)]
use kernaid_windows_pack::diagnostics::{WindowsP0Inputs, diagnose_windows_p0};
use serde::Serialize;
#[cfg(test)]
...
pub fn normalize_dism(stdout: &str, stderr: &str, exit_code: i32) -> Result<String, ()> {
...
pub fn sfc_not_run_projection() -> Result<String, ()> {
...
pub fn derive_storage_identity(volumes_body: &str) -> Result<String, ()> {
    use kernaid_windows_pack::diagnostics::parse_volumes;
    ...
    .map_err(|_| ())?;
    if encoded.len() > kernaid_windows_pack::diagnostics::MAX_INPUT_BYTES {
        return Err(());
    }
    Ok(encoded)
}
...
pub fn validate_projection(collector: &str, body: &str) -> Result<(), ()> {
    ...
    let input = EvidenceInput {
        id: validation_id(collector).ok_or(())?,
        body: body.as_bytes(),
    };
    match collector {
...
#[cfg(test)]
pub fn validate_complete_set(documents: &[(&str, &str)]) -> Result<(), ()> {
    ...
    let input = |collector: &str| -> Result<EvidenceInput<'_>, ()> {
        Ok(EvidenceInput {
            id: validation_id(collector).ok_or(())?,
            body: by_collector.get(collector).ok_or(())?.as_bytes(),
        })
    };
...
mod tests {
    ...
    fn complete_fixture_set_passes_cross_source_validation() {
        ...
        incomplete.pop();
        assert_eq!(validate_complete_set(&incomplete), Err(()));
    }
