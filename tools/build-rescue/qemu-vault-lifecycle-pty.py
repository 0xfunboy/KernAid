...
FIRSTBOOT_RESULT_TIMEOUT_SECONDS = 3600.0
CONTROLLER_TIMEOUT_SECONDS = 4200
...
PROVIDER_PROOF_REPAIR_CHECKPOINTS = (
    ...
    "prepare-terminal",
    "prepare-state",
    "prepare-target-capability-timed-out",
    "prepare-target-capability-identity-changed",
    "prepare-target-capability-unavailable",
    ...
    "prepare-observation-preview",
    "prepare-vault-reserve",
    "prepare-admission-internal",
    "prepare-contract",
    "approve-submit",
    "execute-terminal",
    "execute-state",
...
PROVIDER_PROOF_ROLLBACK_CHECKPOINTS = (
    ...
    "rollback-prepare-error-authority",
    "rollback-prepare-error-target",
    "rollback-prepare-error-lock",
    "rollback-prepare-error-timeout",
    "rollback-prepare-error-vault",
    "rollback-prepare-error-recovery",
    "rollback-prepare-error-unavailable",
    "rollback-contract",
    "rollback-approve",
    "rollback-terminal",
...
try:
    ...
    for report_id,expected_json in REPORTS.items():
        ...
        if envelope["schema"]!="https://schemas.kernaid.dev/v1/signed-report-envelope.json" or envelope["kind"]!="kernaid.signed-report" or envelope["algorithm"]!="Ed25519" or type(envelope["deviceId"]) is not str or re.fullmatch(r"KA-[0-9a-f]{{24}}",envelope["deviceId"]) is None or type(envelope["journalSequence"]) is not int or envelope["journalSequence"]<1 or envelope["payloadMediaType"]!="application/json" or urlsafe(envelope["payloadSha256"],32)!=hashlib.sha256(expected_payload).digest() or urlsafe(envelope["journalEntryHash"],32) is None or urlsafe(envelope["payload"],len(expected_payload))!=expected_payload or urlsafe(envelope["publicKey"],32) is None or urlsafe(envelope["signature"],64) is None:
            raise RuntimeError()
