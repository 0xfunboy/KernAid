...
rescue_not_ready_observed() {
  LC_ALL=C grep -aq '^KERNAID_RESCUE_NOT_READY:' "$log" \
    || LC_ALL=C grep -aqE 'KERNAID_RESCUE_UI_SESSION_FAILURE_V1 stage=(account|user-runtime-mask|xauthority|runtime|process|process-(environment|foreign-display|identity|multiple)|timeout|wait-(xauthority|runtime|process|xauthority-runtime|xauthority-process|runtime-process|xauthority-runtime-process)|internal)[[:space:]]*$' "$log"
}
...
report_tauri_sandbox_failure() {
  ...
  session_marker="$(
    LC_ALL=C tr -d '\r' <"$log" \
      | grep -aoE 'KERNAID_RESCUE_UI_SESSION_FAILURE_V1 stage=(account|user-runtime-mask|xauthority|runtime|process|process-(environment|foreign-display|identity|multiple)|timeout|wait-(xauthority|runtime|process|xauthority-runtime|xauthority-process|runtime-process|xauthority-runtime-process)|internal)$' \
      | tail -n 1 \
      || true
  )"
