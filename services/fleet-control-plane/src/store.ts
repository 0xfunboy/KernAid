      ...
      try {
        this.#transaction(() => {
          this.#database.exec(`
            ...
            INSERT INTO work_orders_v13 SELECT * FROM work_orders;
            DROP TABLE work_orders;
            ALTER TABLE work_orders_v13 RENAME TO work_orders;
            CREATE INDEX work_orders_queue_idx ON work_orders(
              tenant_id, target_device_id, status, created_at, work_order_id
            );
          ...
          const violations = this.#database
            ...
            .all();
          if (violations.length !== 0) {
            throw new Error("Fleet v13 migration violated a foreign key");
          }
        });
      } finally {
        this.#database.exec("PRAGMA foreign_keys = ON");
