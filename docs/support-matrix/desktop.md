...
[workflow run 33300434959](https://github.com/0xfunboy/KernAid/actions/runs/33300434959)
from commit
...
| --- | --- | --- | --- | --- |
| Linux x86-64 | AppImage, DEB, RPM | Runtime-probed, partial | [Linux job 99227432839](https://github.com/0xfunboy/KernAid/actions/runs/33300434959/job/99227432839) built all packages, exercised the production normalized-snapshot command through Tauri IPC on Ubuntu 24.04 and verified the packaged Desk excludes repair UI. It did not install and launch every package format. | Not qualified |
| Windows x86-64 | MSI, NSIS | Build-only, offline runtime bundled | [Windows job 99227432795](https://github.com/0xfunboy/KernAid/actions/runs/33300434959/job/99227432795) built both installers with Tauri `offlineInstaller` WebView2 mode, exercised credential-boundary tests and verified the packaged Desk excludes repair UI. It did not run the native Windows P0 collector set or an installed-app smoke test. | Not qualified |
| macOS Apple silicon | APP, DMG | Runtime-probed, partial | [Apple-silicon job 99227432869](https://github.com/0xfunboy/KernAid/actions/runs/33300434959/job/99227432869) built both packages, ran the native macOS P0 source probe on the hosted runner and verified the packaged Desk excludes repair UI. It did not install the DMG on representative hardware. | Not qualified |
| macOS Intel x86-64 | APP, DMG | Build-only | [Intel job 99227432892](https://github.com/0xfunboy/KernAid/actions/runs/33300434959/job/99227432892) cross-built both packages and verified the packaged Desk excludes repair UI. Native runtime and collector probes were skipped for this target. | Not qualified |
...
- The Windows bundle embeds the WebView2 offline installer. Workflow run
  `33300434959` proves successful MSI and NSIS packaging with that configuration;
  it does not yet prove installation or first launch on representative hardware.
- All four jobs in workflow run `33300434959` passed the packaged-output gate
